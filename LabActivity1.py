    ...
    while True:
        ...
        if result == "Borrowing allowed"
            ...
            if books_requested <= 0:
                ...
                print(f"Success: {name} is allowed to borrow 3 books.")
                successful_borrowers += 1
            else:
                print(f"Success: {name} is allowed to borrow {books_requested} book(s).")
                successful_borrowers += 1
        else:
            print(f"Result for {name}: {result}")

    print("\n")
    print(f"Kiosk Session Ended.")
    print(f"Total students successfully allowed to borrow: {successful_borrowers}")
    print("=")
